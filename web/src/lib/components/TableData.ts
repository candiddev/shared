...
function onDrag(srcID: string, x: number, y: number): void {
  const elNew =
    x === 0 && y === 0
      ...
      : (document.elementFromPoint(x, y) as HTMLElement).closest("tr");

  // Check if elNew is an item and isn't the same as the source
  if (
    elNew !== null &&
    elNew.tagName === "TR" &&
    elNew.parentElement?.tagName === "TBODY"
  ) {
    ...
    if (Drag.isSourceAfter(x === 0 && y === 0 ? Drag.state.y : y, elNew)) {
  ...
  return dragEnd()
    ...
    .catch(() => {
      if (Drag.state.parent !== null) {
        Drag.state.parent.insertBefore(
          el,
          Drag.state.parent.children[Drag.state.position],
        );
      }
    });
}
  ...
  return {
    view: (vnode): m.Children => {
              ...
              data.map((item: string): m.Children => {
                return m(
                  m.route.Link,
                  {
                    ...
                    href:
                      vnode.attrs.linkFormatter === undefined
                        ? ""
      ...
      const c = SetClass({
        TableData: true,
        "TableData--negative":
          vnode.attrs.positive !== undefined &&
          vnode.attrs.positive(vnode.attrs.data) === false,
        "TableData--positive":
          vnode.attrs.positive !== undefined &&
          vnode.attrs.positive(vnode.attrs.data) === true,
        "TableData--right": vnode.attrs.rightAlign === true,
      });

      switch (
        vnode.attrs.type // eslint-disable-line @typescript-eslint/switch-exhaustiveness-check
      ) {
        ...
        case TableDataType.Checkbox:
          if (typeof data === "boolean" || typeof data === "undefined") {
            return m(
              "td.TableData__checkbox",
              m(Icon, {
                icon: data === true ? Icons.CheckYes : Icons.CheckNo,
                id: id,
                onclick: async (e: MouseEvent) => {
                  if (
                    vnode.attrs.checkboxOnclick !== undefined &&
                    AppState.isSessionOnline() &&
                    vnode.attrs.permitted !== undefined &&
                    vnode.attrs.permitted()
                  ) {
                    e.stopPropagation();
                ...
                style: {
                  color: data === true ? "var(--color_secondary)" : undefined,
                },
              }),
            );
          }
            ...
            [
              ...
              m(
                `i.GlobalButtonIconExpand${
                  vnode.attrs.collapsedNames !== undefined &&
                  vnode.attrs.collapsedNames.includes(`${data}`)
                    ? ""
        ...
        case TableDataType.Currency:
          if (typeof data === "number") {
            return m(
              ...
              Currency.toString(
                data,
                vnode.attrs.currencyFormat === undefined
                  ? CurrencyEnum.USD
        ...
        case TableDataType.Drag:
          return m(
            "td.TableData__draggable",
            {
              ...
              ontouchmove: async (e: m.Event<TouchEvent>) => {
        ...
        case TableDataType.Link:
          if (
            vnode.attrs.linkRequireOnline === true &&
            !AppState.isSessionOnline()
          ) {
            return m(
              `td.${c}`,
          ...
          return data === ""
            ...
            : m(
                ...
                [
                  ...
                  m(Icon, {
                    icon: hidden ? Icons.Hide : Icons.Show,
                    style: {
          ...
          if (typeof data === "string") {
            return m(
              ...
              AppState.formatCivilDate(
                Timestamp.fromString(data).toCivilDate(),
              ),
            );
          }
          break;
