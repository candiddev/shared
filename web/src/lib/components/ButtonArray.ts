...
export function ButtonArray(): m.Component<ButtonArrayAttrs> {
  return {
    view: (vnode): m.Children => {
      return m(
        `div.ButtonArray.${Animate.class(Animation.Fade)}`,
        {
        ...
        vnode.attrs.value.map((item) => {
          ...
          const color =
            typeof item !== "string" && item.color !== undefined
              ? item.color
              : "";
          ...
          return m(
            vnode.attrs.valueLinkPrefix === undefined
              ? ("p" as any) // eslint-disable-line @typescript-eslint/no-explicit-any
              : m.route.Link,
            {
              class: SetClass({
                ButtonArray__item: true,
                "ButtonArray__item--small": vnode.attrs.small === true,
                ButtonArray__selected:
                  vnode.attrs.selected !== undefined &&
                  vnode.attrs.selected().includes(id),
              }),
              href:
                vnode.attrs.valueLinkPrefix === undefined
                  ? undefined
              ...
              style: {
                "background-color":
                  ...
                  vnode.attrs.selected().includes(id)
                    ? Color.toHex(color)
                    : undefined,
                "border-color": color === "" ? undefined : Color.toHex(color),
                color:
                  color !== "" &&
                  vnode.attrs.selected !== undefined &&
                  vnode.attrs.selected().includes(id)
            ...
            [
              vnode.attrs.icon !== undefined ||
              (typeof item !== "string" && item.icon !== undefined)
                ? m(Icon, {
                    icon:
                      typeof item !== "string" && item.icon !== undefined
                        ? item.icon
