...
export type ChartCanvasOptions = ChartConfiguration<
  ...
  return {
    oninit: (): void => {
      ...
      if (app !== null) {
        ...
        Chart.defaults.datasets.line.backgroundColor = `${window
    ...
    view: (vnode): m.Children => {
      return m(
        "div.ChartCanvas__chart",
        m(`canvas#${vnode.attrs.id}`, {
          oncreate: (c): void => {
            s = vnode.attrs.data.map((data) => {
              ...
              if (data !== undefined) {
                ...
                if (process.env.NODE_ENV !== "test") {
                  el = (c.dom as HTMLCanvasElement).getContext(
                    "2d",
