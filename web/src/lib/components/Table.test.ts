...
test("Table", async () => {
  ...
  for (let i = 0; i < attrs.data.length; i++) {
    ...
    testing.hasAttribute(
      data,
      "href",
      `#!/test/${item.value}?count=${item.count}`,
    );
  ...
  expect(attrs.sort!().invert).toBeTruthy();
  ...
  expect(document.getElementById("tablediv")!.scrollTop).toBe(1000);
  expect(attrs.sort!().invert).toBeTruthy();
  expect(Object.keys(attrs.tableColumnsNameEnabled())).toHaveLength(4);
