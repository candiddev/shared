  ...
  return {
    view: (vnode): m.Children => {
      return m("div.FormItem", [
        ...
        m(
          "ul.FormItemSelectNested__list",
          {
            id: `form-item-select${StringToID(vnode.attrs.name)}`,
            oncreate: (e) => {
              e.dom.scrollTop =
                document.getElementById(
                  `form-item-option${StringToID(vnode.attrs.name)}-${vnode.attrs.value}`,
                )!.offsetTop - 50; // eslint-disable-line @typescript-eslint/no-non-null-assertion
            },
          },
          vnode.attrs.options.map((option) => {
            return m(
              "li.FormItemSelectNested__option",
              {
                class:
                  vnode.attrs.value === option.id
