...
export const Currency = {
  toNumber: (currency: string): number => {
    let n = parseInt(
      currency.replace(/[((AED)(CHF)(CLP)(MX)(SEK)(NT)(ZAR)R€₹¥₩£$.,\s]/g, ""),
      10,
    );
  ...
  toString: (currency: number, format: CurrencyEnum): string => {
    ...
    if (
      format !== CurrencyEnum.CLP &&
      format !== CurrencyEnum.JPY &&
      format !== CurrencyEnum.KRW &&
      format !== CurrencyEnum.RUB
    ) {
      n /= 100;
    }
