...
test.each([
  ["unknown" as KeyType, true, EncryptionTypeNone],
  [KeyTypeNone, false, EncryptionTypeNone],
  [KeyTypeAES128, false, EncryptionTypeAES128GCM],
  [KeyTypeRSA2048Private, false, EncryptionTypeRSA2048OAEP],
  [KeyTypeRSA2048Public, false, EncryptionTypeRSA2048OAEP],
])(
  "Key %s",
  async (
    keyType: KeyType,
    wantErr: boolean,
    wantEncryptionType: EncryptionType,
  ) => {
    const k = await NewKey(keyType);
    expect(IsErr(k)).toBe(wantErr === true);
    ...
    if (IsErr(k)) {
      ...
      expect(decrypt.type).toBe(keyType);
      ...
      expect(out.keyID).toBe(encrypt.id);
      expect(out.encryption).toBe(wantEncryptionType);
      expect(await decrypt.decrypt(out)).toBe("hello");
    }
  },
);
