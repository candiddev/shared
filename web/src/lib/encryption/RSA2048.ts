...
import {
  ArrayBufferToBase64,
  Base64ToArrayBuffer,
} from "@lib/utilities/ArrayBuffer";

export async function NewRSA2048Key(): Promise<
  { privateKey: string; publicKey: string } | Err
> {
...
export async function importRSA2048Key(
  key: string,
  publicKey?: boolean,
): Promise<CryptoKey | Err> {
  return crypto.subtle
    .importKey(
      publicKey === true ? "spki" : "pkcs8",
      ...
      true,
      [publicKey === true ? "encrypt" : "decrypt"],
    )
    .catch((err) => {
...
export async function rsa2048OAEPEncrypt(
  publicKey: string,
  input: string,
): Promise<string | Err> {
...
export async function rsa2048OAEPDecrypt(
  privateKey: string,
  input: string,
): Promise<string | Err> {
