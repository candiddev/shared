...
describe("AppState", () => {
  test("init", async () => {
    ...
    expect(AppState.parserFormItemAutocomplete.options()).toStrictEqual(
      autocompleteOptions,
    );
    expect(AppState.parserMarkdown.match.test("test")).toBeTruthy();
    expect(AppState.motd()).toBe("hello");
    await AppState.oncreate();
    expect(oncreate).toBeTruthy();
    expect(AppState.preferences().colorAccent).toBe("red");
    expect(AppState.product).toBe("test");
  });
});
