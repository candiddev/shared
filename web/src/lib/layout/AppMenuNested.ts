    ...
    view: (vnode): m.Children => {
      return [
        m(
          "li.AppMenu__entry",
          {
            class: m.route.get().includes(`${vnode.attrs.data.id}`)
          ...
          [
            m(
              ...
              [
                m(Icon, {
                  icon:
                    vnode.attrs.data.icon === undefined ||
                    vnode.attrs.data.icon === ""
                      ? vnode.attrs.icon
                      : vnode.attrs.data.icon,
                  style:
                    vnode.attrs.data.color === undefined
                      ? undefined
                      : {
                          color: Color.toHex(
                ...
                vnode.attrs.findCount !== undefined &&
                vnode.attrs.findCount(vnode.attrs.data) > 0
                  ? m(
                      "span.GlobalCount",
                      vnode.attrs.findCount(vnode.attrs.data),
                    )
                  : [],
              ],
            ),
            vnode.attrs.data.children !== undefined &&
            vnode.attrs.data.children.length > 0
              ? m(
                  `i.AppMenu__arrow.GlobalButtonIconExpand${
                    vnode.attrs.isCollapsed(vnode.attrs.data.id)
                      ? ""
        ...
        AppState.getLayoutAppMenuPath().startsWith(vnode.attrs.pathPrefix) &&
        vnode.attrs.data.children !== undefined &&
        vnode.attrs.data.children.length > 0
          ? m(
              "ul.AppMenu__menu.AppMenu__menu--view",
              {
...
export function AppMenuNested(): m.Component<AppMenuNestedAttrs> {
  return {
    view: (vnode): m.Children => {
      return vnode.attrs.data().map((data) => {
        return m(AppMenuNestedEntry, {
          data: data,
          findCount: vnode.attrs.findCount,
          href: vnode.attrs.href,
