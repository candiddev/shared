...
run-hugo-start () {
	...
	if not_running "candiddev_hugo_${APP_NAME}"; then
...
run-postgresql-start () {
	...
	if not_running candiddev_postgresql; then
...
run-yaml8n-start () {
	...
	for i in "${DIR}"/yaml8n/*; do
		...
		if not_running "candiddev_yaml8n_${name}"; then
